...
class ExpenseGoSupabaseUser extends BaseAuthUser {
  ExpenseGoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> expenseGoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ExpenseGoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
